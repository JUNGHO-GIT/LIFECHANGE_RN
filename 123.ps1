param(
    [string]$OriginRepo = "JUNGHO-GIT/LIFECHANGE_RN",
    [string]$PrivateRepo = "JUNGHO-GIT/LIFECHANGE_RN_PRIVATE",
...
Write-Host ">>> Step 1-1. check if PUBLIC repo exists on GitHub"

$hasPublicRepo = $true
$publicRepoApiUrl = "https://api.github.com/repos/$OriginRepo"
$repoCheckHeaders = @{
    Authorization = "token $Token"
    Accept        = "application/vnd.github+json"
}
...
try {
    Invoke-RestMethod -Uri $publicRepoApiUrl -Method GET -Headers $repoCheckHeaders *> $null
}
catch {
    $hasPublicRepo = $false
    Write-Host ">>> [WARN] PUBLIC repo '$OriginRepo' not found. Skip PUBLIC-related steps."
}
...
if ($hasPublicRepo) {
    ...
    Write-Host ">>> Step 4. skipped (PUBLIC repo not exists)"
}
...
git ls-remote --heads public 2>$null
