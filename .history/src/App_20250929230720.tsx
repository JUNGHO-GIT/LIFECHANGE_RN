...
export const App = () => {
  ...
  useEffect(() => {
	try {
	  const onBackPress = () => (
		webViewRef.current && navigationEnabled ? (
  ...
  const handlerOnMessage = (event: any) => {
	try {
	  ...
	  // 세션아이디는 단일 string
	  parsedData.type === 'sessionId' && AsyncStorage.setItem("sessionId", parsedData.sessionId);

	  // 로케일은 객체
	  parsedData.type === 'localeSetting' && AsyncStorage.setItem("localeSetting", JSON.stringify(parsedData.localeSetting));
	}
	catch (err: any) {
